...
export const name = "list_asyncapi_operations";

export const title = "List AsyncAPI operations";

export const description = buildToolDescription({
  name,
  title,
  summary:
    "List operations with action, operationId, channels, messages, tags, and text fields.",
  inputs: ["`source`: raw YAML/JSON or absolute path to spec file"],
  returns: ["JSON array of operation summary objects."],
...
export const execute = async ({ source }: QueryParams) => {
  try {
    ...
    const { extractOperations } = await import("../../api/parser/extractors.js");
    const doc = await parseToDocument(source);
    const data = extractOperations(doc);
    return {
